...
def compute_metrics(gt_json, pred_json, split='test', total_sample=20):
    ...
    # data loading
    print('loading GT from %s' % gt_json)
    with open(gt_json, 'r') as file: gt_data = json.load(file)
    print('loading results from %s' % pred_json)
    ...
    for pred_len in [10, 20, 50]:
        ...
        # logging header
        print('')
        print('-' * 30 + ' STATS for length of %d ' % pred_len + '-' * 30)
        print(' ' * 5, end=" ")
        print('ADE_%d'%pred_len+' '*2+'FDE_%d'%pred_len+' '*2+'APD_%d'%pred_len+' '*2+'FPD_%d'%pred_len+' '*2+'MsR_%d'%pred_len+' '*2, end=" ")
        print('')
        ...
        for name, meter in stats_meter.items():
            ...
            if cur_index == 0: print('%s: ' % name[-3:], end=" ")
            print('%6.3f ' % final_value, end=" ")
            cur_index += 1
